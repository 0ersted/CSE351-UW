...
void* mm_malloc (size_t size) {
    ...
    // initialize the pointer to the block to be allocated 
    size_t *footer=NULL;
    ...
    // the block is full, we go to the following block
    if(blockSize < reqSize + MIN_BLOCK_SIZE){ 
        ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags | TAG_USED;

        ptrNext = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
        ptrNext->sizeAndTags = ptrNext->sizeAndTags | TAG_PRECEDING_USED;

        //footer = (size_t*) UNSCALED_POINTER_ADD(ptrNext, SIZE(ptrNext->sizeAndTags) - WORD_SIZE);
        //*footer = ptrNext->sizeAndTags;
    } 
    // the block is not full, so we split the current block 
    else{
        ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED;

        ptrNext = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
        ptrNext->sizeAndTags = (size_t)( (blockSize-reqSize) | TAG_PRECEDING_USED ) ;
        
        footer = (size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE);
        *footer = ptrNext->sizeAndTags;

        insertFreeBlock(ptrNext);
    }
...
void mm_free (void *ptr) {
    ...
    // set footer
    footer = ( size_t* ) UNSCALED_POINTER_ADD(blockInfo, payloadSize);
